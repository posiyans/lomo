        ...
        <tr v-if="editStatusShow">
          <td colspan="3">
            ...
            >
              ...
              >
                <template slot-scope="{row}">
                  <span>{{ row.data.price | formatPrice }}</span>
                </template>
              </el-table-column>
              <el-table-column
                label="Назаначение"
          ...
          <td>
            <ol>
              <li v-for="i in invoice.payments" :key="i.id" class="do-not-carry tl gray">
                {{ i.payment_date | moment('DD-MM-YYYY') }} на сумму {{ i.price | formatPrice }} руб
...
<script>
import { fetchInvoiceInfo, updateInvoice, addPaymentForInvoice, deletePaymentForInvoice, changeStatusForInvoice } from '@/api/admin/bookkeping/invoice'
