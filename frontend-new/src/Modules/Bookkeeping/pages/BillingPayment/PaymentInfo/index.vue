<template>
  <div v-if="load" class="app-container">
    ...
    <div>
      Участок:
      <el-button size="mini" type="warning" plain @click="showChangeStead">{{ payment.stead.number }}</el-button>
      ({{ payment.raw_data[5] }})
    ...
    <div>
      Оплата:
      <el-tag type="danger" :effect="payment.type" @click="selectElect()">
        <i v-if="payment.type == 1" class="el-icon-check" />
        Электоэнергия
      </el-tag>
      <el-tag type="success" :effect="payment.type" @click="payment.type = 2">
