      ...
      >
        <template #default="{row}">
          <div v-if="row.last_reading">
            ...
            <div>
              <ShowTime :time="row.last_reading.created_at " format="DD-MM-YYYY" />
            </div>
          </div>
        </template>
      </el-table-column>
      ...
      >
        <template #default="{row}">
          <div v-if="row.installation_date">
            ...
            <ShowTime :time="row.installation_date " format="DD-MM-YYYY" />
          </div>
          <div v-if="row.verification_date">
            <span>
              годен до
            </span>
            <ShowTime :time="row.verification_date" format="DD-MM-YYYY" />
          </div>
        </template>
      </el-table-column>
      <el-table-column
...
import { fetchDevicesForStead, updateDevices } from 'src/Modules/Bookkeeping/api/meteringDeviceAdminApi.js'
import ShowTime from 'components/ShowTime/index.vue'

export default {
  components: { AddDevice, ShowTime },
  props: {
    stead: {
      type: Object,
      default: () => {
