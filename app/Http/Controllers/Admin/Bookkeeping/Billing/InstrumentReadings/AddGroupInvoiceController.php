...
class AddGroupInvoiceController  extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($action && $readings && is_array($readings)) {
            $items = InstrumentReadings::whereIn('id', $readings)->whereNull('invoice_id')->orderBy('created_at', 'asc')->get();
        }
    ...
    public function setInvoiceForReading(InstrumentReadings $reading)
    {
    ...
    public function setInvoiceForGroupReading($items, $date = false)
    {
        ...
        foreach ($items as $reading) {
            if ($stead_id === true) {
                $stead_id = $reading->deviceRegister->stead_id;
            } else if (is_numeric($stead_id) && $stead_id != $reading->deviceRegister->stead_id) {
                $stead_id = false;
            }
            $price = $reading->getPrice();
            $old = $reading->getPreviousReadings();
        ...
        if ($stead_id) {
            ...
            foreach ($types as $type => $item) {
                ...
                if ($invoice) {
                    InstrumentReadings::whereIn('id', $id)->update(['invoice_id' => $invoice->id]);
                    return true;
                }
            }
